$title
$ontext
Time-stamp:     <2023-04-28 15:32:29 st>
First-written:  <2014/03/15>

Chapter 9までのとは別の表現方法
・消費活動を生産活動の一種とみなす方法
・効用生産部門が「効用」という財を生産
・家計はその効用という財を購入する
...
table   SAM     SAM data (benchmark data)
              agr      man      ser      utl      lab      cap       hh
agr            30       10       30       70        0        0        0
man            10       50       20      220        0        0        0
ser            20       40       20       70        0        0        0
utl             0        0        0        0        0        0      360
lab            50       80       50        0        0        0        0
cap            30      120       30        0        0        0        0
hh              0        0        0        0      180      180        0
;

set     row     / agr, man, ser, utl, lab, cap, hh /;
...
display y0, x0, v_a0, vf0, a_x0, a_v0, a_f0, u0, d0, a_d0, m0, u0, c0,
        c_va0, cu0;
...
gamma(i) = (a_d0(i))**(1/sig_c) * p0(i) / cu0;
...
e_cu .. cu =e=
        (sum(i,
            (gamma(i))**(sig_c) * ((1+t_c(i))*p(i))**(1-sig_c))
        )**(1/(1-sig_c));
...
*       単位消費需要
e_a_d(i) .. a_d(i) =e= 
          (gamma(i) * cu / ((1+t_c(i))*p(i)))**(sig_c);

*       財の市場均衡
e_p(i) .. y(i) =e= sum(j, a_x(i,j)*y(j)) + a_d(i)*u;
...
$offtext
model alternative_model 双対アプローチによる別表現
      / e_c.c, e_c_va.c_va, e_y.y, e_v_a.v_a, e_a_x.a_x, e_a_v.a_v,
      e_a_f.a_f, e_cu.cu, e_u.u, e_a_d.a_d, e_p.p, e_p_va.p_va,
      e_p_f.p_f, e_pu.pu, e_m.m, e_x.x, e_vf.vf, e_d.d /;
...
option mcp = path;
alternative_model.iterlim = 0;
solve alternative_model using mcp;
...
alternative_model.iterlim = 1000;
solve alternative_model using mcp;
chk_walras = round(p.m("agr"), 4);
abort$chk_walras "ワルラス法則が満されていません！", chk_walras;
...
$macro calc_results(scn) \
    ...
    results("c_agr",scn) = d.l("agr"); \
    results("c_man",scn) = d.l("man"); \
    results("c_ser",scn) = d.l("ser");
