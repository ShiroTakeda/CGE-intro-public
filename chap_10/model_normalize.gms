$title CGEモデル：内生変数の基準均衡における値を1に規準化したケース
$ontext
Time-stamp:     <2023-04-28 15:32:06 st>
First-written:  <2014/03/15>

・モデルは model_original.gms と同じ。
...
*       消費需要
e_d(i) .. d(i) =e= 
          u0 * u * (gamma(i)*(e/(u0*u))/((1+t_c(i))*p(i)))**(sig_c);

*       財の市場均衡
e_p(i) .. y0(i)*y(i) =e= sum(j, a_x(i,j)*y0(j)*y(j)) + d(i);

*       合成生産要素の市場均衡
e_p_va(i) .. v_a0(i)*v_a(i) =e= a_v(i)*y0(i)*y(i);

*       生産要素の市場均衡
e_p_f(f) .. v_bar(f) =e= sum(i, a_f(f,i)*v_a0(i)*v_a(i));
...
e_m .. m =e= sum(f, p_f(f)*v_bar(f))
             + sum((f,i), t_f(f,i)*p_f(f)*a_f(f,i)*v_a0(i)*v_a(i))
             + sum(i, t_c(i)*p(i)*d(i));

*       投入需要
e_x(j,i) .. x(j,i) =e= a_x(j,i) * y0(i)*y(i);

*       生産要素需要
e_vf(f,i) .. vf(f,i) =e= a_f(f,i) * v_a0(i)*v_a(i);
...
$offtext
model model_original CGEモデル（元々の表現方法）
...
option mcp = path;
model_original.iterlim = 0;
solve model_original using mcp;
...
$macro calc_results(scn) \
    results("u",scn) = u.l; \
    results("y_agr",scn) = y0("agr")*y.l("agr"); \
    results("y_man",scn) = y0("man")*y.l("man"); \
    results("y_ser",scn) = y0("ser")*y.l("ser"); \
