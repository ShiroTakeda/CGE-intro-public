$title 政府を考慮したモデル
$ontext
Time-stamp:     <2023-04-28 15:34:25 st>
First-written:  <2016/03/15>

+ 政府を考慮したモデル
+ モデルは gov_model_A.gms のモデルと基本的に同じ
+ 分析しているシナリオが異なっている
...
e_c_va(i) .. c_va(i) =e=
             (sum(f, beta_v(f,i)**sig_v(i)
                 * ((1+et_f(f,i))*p_f(f))**(1-sig_v(i))))**(1/(1-sig_v(i)));

*       生産における利潤最大化条件
e_y(i) .. c(i) =e= (1 - et_y(i)) * p(i);
...
e_a_f(f,i) ..
          a_f(f,i) =e=
              (beta_v(f,i) * c_va(i) / ((1+et_f(f,i))*p_f(f)))**(sig_v(i));

*       支出関数
e_e .. e =e=
       u * (sum(i,
           (gamma(i))**(sig_c) * ((1+et_c(i))*p(i))**(1-sig_c)))**(1/(1-sig_c));
       
*       消費需要
e_d(i) .. d(i) =e= u * (gamma(i)*(e/u)/((1+et_c(i))*p(i)))**(sig_c);
...
*       政府の所得の定義式
e_m_gov .. m_gov =e=
           sum(i, et_y(i)*p(i)*y(i))
           + sum((f,i), et_f(f,i)*p_f(f)*a_f(f,i)*v_a(i))
           + sum(i, et_c(i)*p(i)*d(i))
           + p_gov*tax_lump;
...
e_tax_lump ..
    (q_gov - q_gov_bar
    )$(fl_g_fix and (not fl_etax_c) and (not sum(f,fl_etax_f(f))) and (not fl_etax_y))
    + (tax_lump - tax_lump_bar)$(fl_g_var or fl_etax_c or sum(f,fl_etax_f(f)) or
        fl_etax_y) =e= 0;
...
e_et_c(i) ..
    et_c(i) =e=
    t_c(i)$(fl_g_var or (not fl_etax_c))
    + etax_c$(fl_g_fix and fl_etax_c);

*       内生的な生産要素税率
e_et_f(f,i) ..
    et_f(f,i) =e=
    t_f(f,i)$(fl_g_var or (not fl_etax_f(f)))
    + (etax_f(f)*t_f(f,i))$(fl_g_fix and fl_etax_f(f));

*       内生的な生産税率
e_et_y(i) ..
    et_y(i) =e=
    t_y(i)$(fl_g_var or (not fl_etax_y))
    + (etax_y*t_y(i))$(fl_g_fix and fl_etax_y);

*       内生的な消費税用
e_etax_c$(fl_g_fix and fl_etax_c) .. q_gov =e= q_gov_bar;

*       内生的な生産要素税用
e_etax_f(f)$(fl_g_fix and fl_etax_f(f)) .. q_gov =e= q_gov_bar;

*       内生的な生産税用
e_etax_y$(fl_g_fix and fl_etax_y) .. q_gov =e= q_gov_bar;
...
model gov_model_B 政府を考慮したモデル / e_c.c, e_c_va.c_va, e_y.y,
      ...
      e_x.x, e_vf.vf,e_p_u.p_u, e_etax_c.etax_c, e_etax_f.etax_f,
      e_etax_y.etax_y, e_et_c.et_c, e_et_f.et_f, e_et_y.et_y /;
...
gov_model_b.iterlim = 0;
option mcp = path;
solve gov_model_b using mcp;
...
display "@ A-1 & bench";
gov_model_b.iterlim = 1000;
solve gov_model_b using mcp;
output_result("A-1","bench");

*       ------------------------------------------------------------
*       一括税以外の税 → 一括税
display "@ A-1 & scn_lump";
t_c(i) = 0;
t_f(f,i) = 0;
t_y(i) = 0;
solve gov_model_b using mcp;
output_result("A-1","scn_lump");
policy_initialize;
...
execute_unload "gov_model_B.gdx", results, results_pc;
