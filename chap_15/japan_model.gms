$title 日本の CGE モデル
$ontext
Time-stamp: <2023-01-20 22:57:11 st>
...
$if not setglobal sim_name $setglobal sim_name default
...
e_c(j)$y0(j) ..
    c(j) =e=
    sum(i$a_x0(i,j), p_a(i)*a_x0(i,j)) + p_va(j)*a_v0(j);

*   合成生産要素生産の単位費用
e_c_va(j) ..
    c_va(j) =e=
    (sum(f, beta_v(f,j)**sig_v(j)
        * ((1+rt_f(f,j))*p_f(f))**(1-sig_v(j))))**(1/(1-sig_v(j)));
...
*   投資財生産の単位費用
e_c_inv ..
    c_inv =e= sum(i$a_inv0(i), p_a(i) * a_inv0(i));

*   政府消費財生産の単位費用
e_c_gov ..
    c_gov =e= sum(i$a_gov0(i), p_a(i) * a_gov0(i));

*   生産の単位収入
e_r_y(j)$y0(j) ..
    r_y(j) =e= sum(i$a_y0(i,j), p_y(i)*a_y0(i,j));
...
*   輸出量
e_x_e(i)$x_e0(i) .. p_e(i) =e= p_ex * p_ew0(i);

*   輸入量
e_x_m(i)$x_m0(i) .. p_ex * p_mw0(i) =e= p_m(i);

*   生産水準
e_y(j)$y0(j) .. c(j) =e= (1 - rt_y(j)) * r_y(j);

*   合成生産要素
e_v_a(j) .. c_va(j) =e= p_va(j);

*   輸出・国内供給配分活動
e_y_de(i)$y_de0(i) .. p_y(i) =e= r_de(i);
...
*   単位生産要素需要
$macro a_f(f,j)  \
    ((beta_v(f,j) * c_va(j) / ((1+rt_f(f,j))*p_f(f)))**(sig_v(j)))

*   単位消費需要
$macro a_d(i)  ((gamma(i) * c_u / ((1+rt_c(i))*p_a(i)))**(sig_c))

*   Armington統合の単位国内財需要
$macro a_ad(i)  ((alpha_ad(i) * c_a(i) / p_d(i))**(sig_dm(i)))

*   Armington統合の単位輸入財需要
$macro a_am(i)  ((alpha_am(i) * c_a(i) / ((1+rt_m(i))*p_m(i)))**(sig_dm(i)))

*   単位輸出供給
$macro a_es(i)  ((p_e(i) / (delta_es(i) * r_de(i)))**(eta_de(i)))

*   単位国内供給
$macro a_ds(i) ((p_d(i) / (delta_ds(i) * r_de(i)))**(eta_de(i)))

*   ------------------------------------------------------------
*   市場均衡条件

*   生産物価格
e_p_y(i)$y_de0(i) .. sum(j$a_y0(i,j), a_y0(i,j)*y(j)) =e= y_de(i);

*   国内財の価格
e_p_d(i)$a_ad0(i) .. a_ds(i) * y_de(i) + end_hh(i) =e= a_ad(i) * q_a(i);

*   輸出財の価格
e_p_e(i)$x_e0(i) .. a_es(i) * y_de(i) =e= x_e(i);

*   輸入財の価格
e_p_m(i)$x_m0(i) .. x_m(i) =e= a_am(i) * q_a(i);

*   Armington財の市場均衡
e_p_a(i) .. q_a(i) =e= sum(j$a_x0(i,j), a_x0(i,j)*y(j))
        + (a_d(i)*u)$a_d0(i)
        + (a_inv0(i)*q_inv)$a_inv0(i)
        + (a_gov0(i)*q_gov)$a_gov0(i);

*   合成生産要素の市場均衡
e_p_va(j) .. v_a(j) =e= a_v0(j)*y(j);

*   生産要素の市場均衡
e_p_f(f) .. v_bar(f) =e= sum(j$a_f0(f,j), a_f(f,j)*v_a(j));

*   効用の市場均衡
e_p_u ..    p_u * u =e= m_d;

*   投資財の市場均衡
e_p_inv .. q_inv - q_inv_bar =e= 0;
...
*   税収（一括税以外）
e_tax_all .. tax_all =e=
         sum((f,j)$a_f0(f,j), rt_f(f,j)*p_f(f)*a_f(f,j)*v_a(j))
         + sum(i$x_m0(i), rt_m(i)*p_m(i)*x_m(i))
         + sum(i$a_d0(i), rt_c(i)*p_a(i)*a_d(i)*u)
         + sum(j$y0(j), rt_y(j)*r_y(j)*y(j))
         + sum(f$v_bar(f), rt_inc(f)*p_f(f)*v_bar(f));

*   家計の要素所得（税引き前）
e_m_bf .. m_bf =e= sum(f, p_f(f)*v_bar(f));

*   家計の所得（税引き後）
e_m .. m =e= sum(f, (1-rt_inc(f))*p_f(f)*v_bar(f))
       + sum(i, p_d(i)*end_hh(i))
       + p_gov*tax_lump;

*   政府の所得
e_m_gov .. m_gov =e= tax_all - p_gov*tax_lump;

*   消費に支出される所得
e_m_d .. m_d =e= m - p_inv*q_inv - p_ex*ts;

*   ------------------------------------------------------------
*   その他

*   一括税
e_tax_lump .. q_gov =e= q_gov_bar;
...
$macro calc_results(scn) \
    chk_walras(scn) = round(p_d.m("agr"), 2); \
    results("u",scn) = u + eps; \
    results("q_inv",scn) = q_inv + eps; \
    results("q_gov",scn) = q_gov + eps; \
    results("pricon",scn) = sum(i, (1+rt_c0(i))*p_a0(i)*a_d(i)*u) + eps; \
    results("invest",scn) = sum(i, p_a0(i)*a_inv0(i)*q_inv) + eps; \
    results("govcon",scn) = sum(i, p_a0(i)*a_gov0(i)*q_gov) + eps; \
    results("export",scn) = sum(i, p_e0(i)*x_e(i)) + eps; \
    results("import",scn) = sum(i, (1+rt_m0(i))*p_m0(i)*x_m(i)) + eps; \
    results("tot",scn) = p_e("agr") / p_m("agr") + eps; \
    results("ts",scn) = ts + eps; \
    results("m_d",scn) = m_d / p_u ; \
    results("gdp",scn) = \
