$title ニュメレールの設定についてのシミュレーション
$ontext
Time-stamp:     <2023-04-28 15:35:51 st>
...
+ 元々は p.fx("agr") = 1; と設定している。
+ これを p.fx("agr") = 2; に変更する。

+ Chapter 2で説明したように、このモデルは価格について0次同次の性質が成り
　立つ。
+ ニュメレールの価格を2倍にしたとすると、全ての名目価格が2倍に変化する
　だけで、数量変数の値は全く変化しないはず。これを確認する。
$offtext
p.fx("agr") = 2;
...
execute 'gdxxrw i=ge_zero_h_check.gdx o=ge_zero_h_check.xlsx epsout=0 par=results rng=results!A1 rdim=1 cdim=1';
execute 'gdxxrw i=ge_zero_h_check.gdx o=ge_zero_h_check.xlsx epsout=0 par=results_pc rng=results_pc!A1 rdim=1 cdim=1';
