$title Calibrated share formでの表現
$ontext
Time-stamp:     <2023-04-28 15:33:37 st>
...
e_c(i) .. c(i) =e=
          c0(i)
          * prod(j, (p(j)/p0(j))**(sh_x(j,i)))
          * (p_va(i)/p_va0(i))**(sh_v(i));
...
e_c_va(i) ..
    ...
    c_va0(i)
    * prod(f,
        (((1+t_f(f,i))*p_f(f))/((1+t_f0(f,i))*p_f0(f)))**(sh_f(f,i)));
...
e_a_v(i) .. a_v(i) =e= a_v0(i)
            * ((c(i) / c0(i)) / (p_va(i) / p_va0(i)));

*       単位生産要素需要
e_a_f(f,i) ..
          a_f(f,i) =e= a_f0(f,i)
          * ((c_va(i) / c_va0(i)) /
              (((1+t_f(f,i))*p_f(f)) / ((1+t_f0(f,i))*p_f0(f))));

*       支出関数
e_e .. e =e=
       u * (e0/u0)
       * prod(i, (((1+t_c(i))*p(i)) / ((1+t_c0(i))*p0(i)))**(sh_c(i)));

*       消費需要
e_d(i) .. d(i) =e=
          d0(i) * ((e/e0) / (((1+t_c(i))*p(i)) / ((1+t_c0(i))*p0(i))));
