$title Harberger conventionのチェック
$ontext
Time-stamp:     <2023-04-28 15:34:13 st>
First-written:  <2014/03/15>

+ Harberger conventionを利用しないケース
+ つまり、基準均衡における価格を1と仮定しないケース

+ 他はharberger_check.gmsと全く同じ。

チェック
+ harberger_check.gmsと結果がどう違うかをチェックする。
...
$macro calc_results(sc) \
    results("u",sc) = u.l; \
    results("y_agr",sc) = y.l("agr"); \
    results("y_man",sc) = y.l("man"); \
    results("y_ser",sc) = y.l("ser"); \
    results("v_y_agr",sc) = p.l("agr")*y.l("agr") / p.l("agr"); \
    results("v_y_man",sc) = p.l("man")*y.l("man") / p.l("agr"); \
    results("v_y_ser",sc) = p.l("ser")*y.l("ser") / p.l("agr"); \
    results("d_agr",sc) = d.l("agr"); \
    results("d_man",sc) = d.l("man"); \
    results("d_ser",sc) = d.l("ser");
...
set     scn     / cap_up, tax_c /
        var     / u, y_agr, y_man, y_ser,
                  v_y_agr, v_y_man, v_y_ser,
                  d_agr, d_man, d_ser /;

results_pc(var,scn)$results(var,"bench")
    = 100*(results(var,scn)/results(var,"bench") - 1);
