$title 貿易のある一地域モデル
$ontext
Time-stamp:     <2023-04-28 15:36:10 st>
...
model one_region One region trade model / e_c.c, e_c_va.c_va,
      ...
      e_p_e.p_e, e_p_m.p_m, e_p_ex.p_ex, e_ts.ts, e_m.m, e_m_d.m_d /;
...
$macro calc_results(scn) \
    ...
    results("d_man",scn) = a_ad.l("man")*q_a.l("man"); \
    results("d_ser",scn) = a_ad.l("ser")*q_a.l("ser"); \
...
*       ============================================================
*       以下では為替レート固定して同じシミュレーションをする。
$ontext
+ fl_fex に非ゼロを設定すると為替レート（p_ex/p_uの値）を固定。
+ 貿易収支 ts は可変になる。
...
calc_results("rt_m_alt");

*       ------------------------------------------------------------
*       結果（まとめ）

set scn_ / cap_i_fex, rt_c_fex, rt_m_fex, cap_i_alt, rt_c_alt, rt_m_alt /;
...
execute_unload "one_region_alt.gdx", results, results_pc;
