$title 貿易のある一地域モデル
$ontext
Time-stamp:     <2023-04-28 15:36:22 st>
...
*       --------------------------------------------------------------
*       交易条件（TOT）が改善するシミュレーション
$ontext
交易条件を改善するには
+ 輸出財の国際価格が上昇する
+ 輸入財の国際価格が低下する
のどちらか（あるいは両方）が起こればよい。

ここでは両方考える。1.1倍というのは適当に考えた値で特に意味はない。
$offtext
p_ew0(i) = 1.1;
p_mw0(i) = 1/1.1;
...
*       --------------------------------------------------------------
*       交易条件（TOT）が悪化するシミュレーション
$ontext
交易条件を悪化させるには、改善の場合と逆を考えればよい。
$offtext
p_ew0(i) = 1/1.1;
p_mw0(i) = 1.1;
...
set scn / bench, tot_a, tot_b, tot_a_fex, tot_b_fex /;
...
execute_unload "one_region_tot.gdx", results, results_pc;
